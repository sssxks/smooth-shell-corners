...
import {bindAdjustment, bindBoolean, bindStringList, createSpinRow} from './widgets.js';
...
export function addCornerPreferences(win: Adw.PreferencesWindow, settings: Gio.Settings): void {
    ...
    bindBoolean(settings, 'fill-padding', fillRow);
    ...
    for (const [side, key] of [
        ...
        const r = createSpinRow(side, '', 0, 100, 1);
    ...
    const bwRow = createSpinRow(_('Width'), '', -15, 15, 1);
    ...
    bindBoolean(settings, 'keep-rounded-maximized', maxRow);
    behGroup.add(maxRow);
    ...
    bindBoolean(settings, 'keep-rounded-fullscreen', fullRow);
...
export function addApplicationPreferences(win: Adw.PreferencesWindow, settings: Gio.Settings): void {
    ...
    bindBoolean(settings, 'remove-native-radius', nativeRow);
    ...
    bindBoolean(settings, 'skip-libadwaita-app', adwRow);
    ...
    bindBoolean(settings, 'skip-libhandy-app', handyRow);
    ...
    bindBoolean(settings, 'whitelist-mode', whitelistRow);
    ...
    const unbindList = bindStringList(settings, 'blacklist', textView.buffer);
...
export function addAboutPreferences(
    ...
    bindBoolean(settings, 'debug-mode', debugRow);
